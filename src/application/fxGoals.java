...
public class fxGoals {
	...
	public static void open() {
		...
		sGoals.setOnCloseRequest(event -> {
			if (sgEdit != null && sgEdit.isShowing())
				sgEdit.close();
		});
	...
	private static void editDiv(Goal g) {
		...
		if (g == null)
			sgEdit.setTitle("Create Goal");
		else
			sgEdit.setTitle("Edit Goal");
		...
		if (g != null) {
			...
			if (g.time != null)
				teTime.setText(g.time.toString());
			teTime.requestFocus();
		} else
			teDiv.requestFocus();
	}
		...
		if (teDiv.getText() == "") {
			...
			if (t.error == 1) {
				...
				if (t.error == 0)
					v.time = t;
				for (Goal a : Pace.goals)
					if (a.division.toLowerCase().equals(v.division.toLowerCase())) {
	...
	private static void importDivs() {
		...
		for (Team t : Pace.teams)
			if (!strDivs.contains(t.division))
				strDivs.add(t.division);
		List<Goal> goals = new ArrayList<Goal>();
		if (Pace.goals != null)
			for (Goal g : Pace.goals)
				if (strDivs.remove(g.division))
					goals.add(g);
