...
public class TimeTest {
     ...
     * <br>Creates random values using {@link Util#randomTimeValue()}, creates
     ...
     * <ul><li>Creates a new time value using the {@link Util#randomTimeValue()} constructor, pruning off the last 3 digits,
    ...
    public void timeFromString() {
        for (int i = 0; i < NUM_TRIALS; i++) {
            long val = ((new Time(Util.randomTimeValue()).getValue()) / 1000) * 1000;
    ...
    public void differenceNull() {
        ...
        assertNull(Time.difference(Util.randomTime(), null));
        assertNull(Time.difference(null, Util.randomTime()));
    }
    ...
    public void difference() {
        for (int i = 0; i < NUM_TRIALS; i++) {
            long a = Util.randomTimeValue(), b = Util.randomTimeValue();
            long difference = b - a;
    ...
    public void subtract() {
        assertNull(Util.randomTime().subtract(null));
        for (int i = 0; i < NUM_TRIALS; i++) {
            long a = Util.randomTimeValue(), b = Util.randomTimeValue();
            long difference = a - b;
